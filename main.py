...
def start_keyboard_pre():
    ...
    rows.append([InlineKeyboardButton("افزودن ربات به گروه ➕", url="https://t.me/DareGushi1_BOT?startgroup=true")])
    rows.append([InlineKeyboardButton("ارتباط با پشتیبان 👨🏻‍💻", url="https://t.me/OLDKASEB")])
    return InlineKeyboardMarkup(rows)

def start_keyboard_post():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("افزودن ربات به گروه ➕", url="https://t.me/DareGushi1_BOT?startgroup=true")],
...
START_TEXT = (
    ...
    " @RHINOSOUL_TM صفر تا صد هر سرویس "
)
...
async def group_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• حالت اینلاین: @{BOT_USERNAME or 'DareGushi_BOT'} <متن> @username  یا فقط @{BOT_USERNAME or 'DareGushi_BOT'} برای نمایش مخاطبین اخیر."
    )
    rows = [[InlineKeyboardButton("✍️ ارسال متن در خصوصی", url=f"https://t.me/{BOT_USERNAME or 'DareGushi_BOT'}?start=go")]]
    if len(MANDATORY_CHANNELS) >= 1:
        rows.append([InlineKeyboardButton("عضویت در کانال یک", url=f"https://t.me/{MANDATORY_CHANNELS[0]}")])
    if len(MANDATORY_CHANNELS) >= 2:
        rows.append([InlineKeyboardButton("عضویت در کانال دو", url=f"https://t.me/{MANDATORY_CHANNELS[1]}")])
...
async def on_inline_show(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    allowed = (user.id == sender_id) or (receiver_id and user.id == receiver_id) or ((user.username or "").lower() == (recv_un or "")) or (user.id == ADMIN_ID)
    if not allowed:
        await cq.answer("فضولی نکن سرت تو لاک خودت باشه بار بعد فحش میدما", show_alert=True)
        return
...
async def on_reshow_whisper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cq = update.callback_query
    user = update.effective_user
    try:
        _, wid = cq.data.split(":")
    ...
    async with pool.acquire() as con:
        w = await con.fetchrow("SELECT text, sender_id, receiver_id FROM whispers WHERE id=$1;", wid)
    if not w:
        await cq.answer("نجوا یافت نشد.", show_alert=True)
        return

    allowed = user.id in (w["sender_id"], w["receiver_id"], ADMIN_ID)
    if not allowed:
...
async def on_replyto_whisper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cq = update.callback_query
    user = update.effective_user
    try:
        _, target_id, group_id = cq.data.split(":")
        ...
        await con.execute(
            """INSERT INTO pending (sender_id, group_id, receiver_id, created_at, expires_at)
               ...
               ON CONFLICT (sender_id) DO UPDATE SET
                 group_id=$2, receiver_id=$3, created_at=NOW(), expires_at=$4;""",
            user.id, group_id, target_id, FAR_FUTURE
        )
    ...
    await context.bot.send_message(
        user.id,
        f"⌛️ در انتظارِ متنِ نجوای شما…\n"
        f"هدف: {mention_html(target_id, await get_name_for(target_id))} در «{group_link_title((await context.bot.get_chat(group_id)).title)}»\n"
...
async def group_trigger(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.reply_to_message is None:
        warn = await msg.reply_text(
            ".روی پیام کاربر مورد نظر دستور نجوا/درگوشی/سکرت را ریپلای کنید\n\n"
            " @RHINOSOUL_TM صفر تا صد هر سرویس "
        )
    ...
    guide = await context.bot.send_message(
        chat_id=chat.id,
        text=("لطفاً متن نجوای خود را در خصوصی ربات ارسال کنید: @{BOT}").format(BOT=BOT_USERNAME or "DareGushi_BOT"),
        reply_to_message_id=msg.reply_to_message.message_id,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✍️ارسال متن در پیوی ربات", url=f"https://t.me/{BOT_USERNAME or 'DareGushi1_BOT'}?start=go")]])
    )
    async with pool.acquire() as con:
...
async def on_checksub_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_member_required_channel(context, cq.from_user.id):
        ...
        await cq.edit_message_text(
            "✅ عضویت تایید شد. به پیوی ربات برو و متن نجوا را بفرست (فقط متن).",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("✍️ ارسال متن در پیوی ربات", url=f"https://t.me/{BOT_USERNAME or 'DareGushi_BOT'}?start=go")]]
            )
        )
        try:
...
async def private_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt in ("راهنما", "help", "Help"):
        await update.message.reply_text(
            ...
            f"@{BOT_USERNAME or 'BotUsername'} <متن نجوا> @username  یا فقط @{BOT_USERNAME or 'BotUsername'} برای مخاطبین اخیر.\n"
...
if row and update.message.text:
    ...
    try:
        ...
        # 2) اعلان گروه + دکمه
        notify_text = (
            f"{mention_html(receiver_id, receiver_name)} | شما یک نجوا (غیبت) دارید! \n"
            f"👤 از طرف: {mention_html(sender_id, sender_name)}"
        )
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔒 نمایش نجوا(غیبت)", callback_data=f"showid:{w_id}")]])
        ...
        await update.message.reply_text(" @RHINOSOUL_TM صفر تا صد هر سرویس ")
        return
...
async def secret_report(context: ContextTypes.DEFAULT_TYPE, group_id: int,
    ...
    msg = (
        f"📥 نجوا جدید در گروه :{group_title} (ID: {group_id})\n"
        f"👤 فرستنده:{s_label}\n"
        f"🎯 گیرنده:{r_label}\n"
        f"متن: {text}"
    )
...
async def send_whisper(context, sender_id: int, text: str, row):
    group_id = int(row["group_id"])
    receiver_id = int(row["receiver_id"])
    ...
    notify_text = (
        f"{mention_html(receiver_id, receiver_name)} | شما یک نجوا دریافت کردید!\n"
        f"👤 از طرف: {mention_html(sender_id, sender_name)}"
    )
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔒 نمایش نجوا", callback_data=f"showid:{w_id}")]
    ])
    sent = await context.bot.send_message(
        chat_id=group_id,
        text=notify_text,
    ...
    run = await get_username_for(receiver_id)
    await upsert_contact(sender_id, receiver_id, run, receiver_name)
    ...
    await context.bot.send_message(
        sender_id,
        f"✅ نجوای شما برای {mention_html(receiver_id, receiver_name)} ارسال شد.\n"
        f"در گروه «{group_title}» منتظر واکنش باشید😉 @RHINOSOUL_TM ",
        parse_mode=ParseMode.HTML
    )
...
async def on_confirm_whisper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cq = update.callback_query
    user = update.effective_user
    if not cq.data.startswith("confirm:"):
        return

    text = re.sub(r"^confirm:", "", cq.data)
    text = text and text.strip()
    if not text:
        await cq.answer("متن خالی است.", show_alert=True)
        return
    ...
    async with pool.acquire() as con:
        row = await con.fetchrow("SELECT * FROM pending WHERE sender_id=$1 AND expires_at>NOW();", user.id)
    if not row:
        await cq.answer("پندینگ منقضی شده یا وجود ندارد.", show_alert=True)
        return

    # ادامه همان منطق ثبت whisper و ارسال پیام گروهی
    # می‌تونی تابع ارسال نهایی رو جدا کنی و اینجا صدا بزنی
    await cq.answer("در حال ارسال نجوا…")
    await send_whisper(context, user.id, text, row)

async def on_edit_whisper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cq = update.callback_query
    user = update.effective_user

    # فقط اگر پندینگ فعال هست
    async with pool.acquire() as con:
        row = await con.fetchrow("SELECT * FROM pending WHERE sender_id=$1 AND expires_at>NOW();", user.id)
    if not row:
        await cq.answer("پندینگ منقضی شده یا وجود ندارد.", show_alert=True)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(on_confirm_whisper, pattern=r"^confirm:.+"))
    app.add_handler(CallbackQueryHandler(lambda u,c: u.callback_query.answer("لغو شد."), pattern=r"^cancel$"))
    app.add_handler(CallbackQueryHandler(on_edit_whisper, pattern=r"^editwhisper$"))
    app.add_handler(CallbackQueryHandler(on_reshow_whisper, pattern=r"^reshow:\d+$"))
    app.add_handler(CallbackQueryHandler(on_replyto_whisper, pattern=r"^replyto:\d+:-?\d+$"))
